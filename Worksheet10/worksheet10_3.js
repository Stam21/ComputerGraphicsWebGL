...
function render(gl, program, model, drawingParams) {
    ...
    let y = q_rot.apply(vec3(0, 1, 0));
    let x = q_rot.apply(vec3(1, 0, 0));
    let e = q_rot.apply(vec3(0, 0, eyeDistance));


    let c = [scale(lookAtDisplacement[0], x), scale(lookAtDisplacement[1], y)].reduce(add)
    e = [e, c].reduce(add)

    var modelViewMatrix = [
        lookAt(e, c, y), // quaternion based transformations
        translate(0, -1, 0),
    ].reduce(mult);
    ...
    canvas.onmousemove = ev => {
        ...
        if (dragging) {
            ...
            if (mode==1) {
                ...
                // Panning
                let displacement = scale(Math.max(5, 0.25 * eyeDistance), vec2(subtract(lastPos, curPos)));
                lookAtDisplacement[0] += displacement[0];
                lookAtDisplacement[1] += displacement[1];
            }
