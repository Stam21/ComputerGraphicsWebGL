...
var OBJDoc = function (fileName) {
  ...
  this.texCoords = new Array(0);  // Initialize the property for Texture
}
...
OBJDoc.prototype.parseFace = function (sp, materialName, vertices, reverse) {
  ...
  if (face.vIndices.length > 3) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      newTIndices[i * 3 + 0] = face.texIndices[0]; // Assuming the same texture for each vertex
      newTIndices[i * 3 + 1] = face.texIndices[i + 1];
    }
    face.vIndices = newVIndices;
    face.nIndices = newNIndices;
    face.texIndices = newTIndices;
  }
  ...
  for (var i = 0; i < this.objects.length; i++) {
    ...
    for (var j = 0; j < object.faces.length; j++) {
      ...
      for (var k = 0; k < face.vIndices.length; k++) {
        ...
        var tIdx = face.texIndices[k];
        if (tIdx >= 0) {
            var texCoord = this.texCoords[tIdx];
            texCoords[vIdx * 2 + 0] = texCoord.x;
            texCoords[vIdx * 2 + 1] = texCoord.y;
        } else {
            // If no texture coordinate is provided, set default values
