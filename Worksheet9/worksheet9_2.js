...
function render(gl, program, program2, model, model2, drawingParams) {
    ...
    if (drawingParams) {
        ...
        let lightX = Math.sin(theta);        
        let lightY = 2;
        let lightZ = -2 + Math.cos(theta);

        let projectionMatrix = mat4(1);
        projectionMatrix[3][3] = 0;
        projectionMatrix[3][1] = 1 / -(lightY - (-1.002));
        let modelView = [
        ...
        gl.uniformMatrix4fv(uLocationMO, false, flatten(modelView));
        
        gl.uniform1i( gl.getUniformLocation(program2, "visibility"),false);
        gl.depthFunc(gl.GREATER);
        gl.drawElements(gl.TRIANGLES, drawingParams.indices.length, gl.UNSIGNED_INT, 0);
       
        mvMatrix = lookAt(eye, at, up);

        gl.uniform1i( gl.getUniformLocation(program2, "visibility"),true);
